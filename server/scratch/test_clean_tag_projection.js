...
(async () => {
  ...
  console.log('--- TESTING CLEAN TAG PROJECTION EXPORT ---');
  const parsed = await parseFile(srcPath);

  // Load target text snippets from target file, stripping raw HTML tags to get clean translated text
  const tgtHtmlPath = 'C:\\Users\\divya\\Desktop\\matecat\\client\\src\\testing\\SCUBL_DIGITAL_LOAN_AGREEMENT_PROD_pa (1).html';
  let targetCleanSnippets = [];
  if (fs.existsSync(tgtHtmlPath)) {
    ...
    $tgt('td, p, div, li').each((_, el) => {
      ...
      if (txt.length > 0) targetCleanSnippets.push(txt);
    });
  }

  // Populate segments: Use clean target text and project exact source tags!
  const finalSegments = parsed.segments.map((seg, idx) => {
    ...
    const cleanTargetText = targetCleanSnippets[idx] || cleanSource;
    // Project source tags onto clean target text!
    const projectedTarget = projectSourceTagsOntoTarget(seg.source, cleanTargetText);
  ...
  console.log(`Successfully generated clean tag projected target file at ${outPath}`);
  ...
  checkTagBalance(exportedHtml, 'PROJECTED TARGET');
