...
const {
  ...
  splitByPunctuation,
  extractSegmentTags,
} = require("../src/utils/parsers/segmentationUtils");
...
async function originalParse(filePath) {
  ...
  // Preprocess body
  // (In original, wrapInlineSiblings was called on body)
...
// Current new parser is in ../src/utils/parsers/htmlParser.js
const { parseFile: parseHtmlNew } = require("../src/utils/parsers/htmlParser");

async function run() {
  const filePath = "C:/Users/divya/Downloads/Meet Our Leadership Team _ Airtel Payments Bank.html";
  
  const origSegments = await originalParse(filePath);
  const newResult = await parseHtmlNew(filePath);

  console.log(`Original Parser segments: ${origSegments.length}`);
  console.log(`New Parser segments: ${newResult.segments.length}`);

  const badOriginal = origSegments.filter(s => s.source.includes("__SEG_"));
  const badNew = newResult.segments.filter(s => s.source.includes("__SEG_"));

  console.log(`Original Bad segments: ${badOriginal.length}`);
  console.log(`New Bad segments: ${badNew.length}`);
}
